...
unsigned char* SHA256(unsigned char* data, unsigned int data_len) {
    //printf("str len\n");
...
unsigned char* SHA256x2(unsigned char* data, unsigned int data_len) {
    unsigned char* first_round = SHA256(data, data_len);
    unsigned char* second_round = SHA256(first_round, 32);
    free(first_round);
    return second_round;
}